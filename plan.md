...
- [ ] app/config.py: pydantic-settings (API_KEY, BASE_URL, LLM_MODEL, ACCESS_CODE, MOCK_LLM=false, SESSION_TTL_SECONDS=3600, ENVIRONMENT, LOG_LEVEL) behind get_settings(); nothing else reads os.environ
- [ ] app/main.py: create_app() factory — fails fast if API_KEY missing, structlog configured, nothing runs at import time except create_app()
- [ ] GET /health (no access code required)
- [ ] Static mount serving frontend/dist (placeholder build for now)
...
- [ ] KB: Claude drafts docs/cadre-kb.md from https://www.cadreai.com/ — evergreen core only, extract don't paraphrase, one section per topic with its source URL, never invent (if a page doesn't state it, it doesn't go in). STOP and show Paola the full diff for review BEFORE writing the file — her approval is what makes the KB trusted. The MANUALLY SOURCED section stays untouched. Coverage target: the 6 scenarios (portal access, AI Maturity Index, LLM choice / data security posture)
- [ ] app/data/knowledge.py: load + parse sections into KnowledgeSection(id, title, content) at startup, fail fast on missing file or empty section
- [ ] app/data/repository.py: KnowledgeSource.retrieve(query) (static impl returns everything, query param is the RAG seam) + get_booking_link(). Only this file imports knowledge.py
- [ ] app/core/prompt_builder.py: build_system_prompt(sections) = persona + sections + boundaries + fallback protocol; never imports app/data
- [ ] app/core/ai_service.py: openai SDK client pointed at BASE_URL, get_response(messages), fallback-marker parser (find, strip, return reason), AIUnavailableError / AIRateLimitError mapped from SDK errors, ~500 output-token cap, MOCK_LLM=true returns canned replies offline
- [ ] app/sessions: Message/Session models, SessionStore ABC (create/get/save/delete), in-memory store (dict + lock + TTL check on read), SessionManager on top; store keeps full history
- [ ] app/api: schemas (ChatRequest{message}, ChatResponse{chat_id, reply, fallback|null}), routes POST /api/v1/chat, POST /api/v1/chat/{id}/messages, GET /api/v1/chat/{id}; X-Access-Code check on all /api/v1/* (401 ACCESS_DENIED), trim to last 10 messages before core, map errors 502 / 429 / 404 UNKNOWN_CHAT, all errors {code, message}
- [ ] UI: Gate.jsx (code → sessionStorage), api.js (all fetches, X-Access-Code header, 401 → back to gate), App.jsx (messages, input, loading, error banner)
- [ ] pytest: prompt contains KB + boundaries, marker parser, session TTL expiry, access gate 401, route status codes + error shape (TestClient, faked LLM client)
